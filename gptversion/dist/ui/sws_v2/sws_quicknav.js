(function(){
  ...
  async function createPanel(opts){
    ...
    const INDENT_STEP = 54; // ширина кнопки розгортання + 10px
    ...
    const makeSpaceNode = (s, depth=0, num='')=>{
      ...
      // Indent only the label button, keep caret aligned across depths.
      row.style.marginLeft = (depth*INDENT_STEP)+'px';
      row.style.width = `calc(100% - ${depth*INDENT_STEP}px)`;
      b.type='button';
      b.onclick = async ()=>{
        if(onGoSpace){
